...
// --- CORS FIX (Very Important for Vercel → Railway communication) ---
app.use(cors({
    origin: "*",
...
wss.on('connection', async (ws, req) => {
    ...
    ws.on('message', async (msg) => {
        try {
            ...
            const state = activeConnections.get(userId);
            if (!state) return;
...
const updatePortfolio = (portfolio, ticker, quantity, type, price) => {
    ...
    if (type === 'BUY') {
        ...
        if (holding) {
            ...
            const newCost = holding.quantity * holding.avgPrice + totalCost;
...
const processTrade = async (userId, ticker, quantity, type) => {
    ...
    if (type === 'BUY') {
        if (user.cash >= value) {
            user.cash -= value;
            const updated = updatePortfolio(user.portfolio, ticker, quantity, 'BUY', price);
            user.portfolio = updated.portfolio;
            tradeMsg = updated.tradeMsg;
        } else {
            tradeMsg = `Trade failed: Insufficient cash.`;
        }
    } else if (type === 'SELL') {
        const holding = user.portfolio.find(h => h.ticker === ticker);
        if (holding && holding.quantity >= quantity) {
            user.cash += value;
            const updated = updatePortfolio(user.portfolio, ticker, quantity, 'SELL', price);
            user.portfolio = updated.portfolio;
            tradeMsg = updated.tradeMsg;
        } else {
            tradeMsg = `Trade failed: You do not own enough shares.`;
        }
    }
...
const handleAlert = async (userId, ticker, threshold) => {
    ...
    if (conn) {
        conn.ws.send(JSON.stringify({
            type: "ALERT_SET_SUCCESS",
            tradeMsg: `Successfully set alert for ${ticker}.`,
            activeAlerts: Object.fromEntries(user.alerts)
        }));
    }
};

const checkAndTriggerAlerts = async (ticker, price) => {
...
// --- Utility functions ---
const formatPortfolioForClient = (portfolio) => {
    const out = {};
    portfolio.forEach(h => {
        if (h.quantity > 0) {
            out[h.ticker] = {
...
const calculateRiskMetrics = (portfolio) => {
    const count = portfolio.filter(h => h.quantity > 0).length;
    const total = SUPPORTED_STOCKS.length;
    let diversificationScore = DIVERSIFICATION_SCORE_BASE;

    if (count > 0) {
        diversificationScore = Math.min(100, (count / total) * 100 + 50);
    }
...
// --- Price Simulation ---
const simulatePriceUpdate = () => {
    const marketChange = (Math.random() - 0.5) * 0.1;
    MARKET_INDEX += MARKET_INDEX * marketChange;
...
const broadcastPrices = async () => {
    ...
    for (const [userId, conn] of activeConnections.entries()) {
        ...
        subscribed.forEach(ticker => {
            if (newPrices[ticker]) {
                sendPrices[ticker] = newPrices[ticker];
                shouldSend = true;
            }
        });
